...
{
    public class GetBySearchSkillCategoryRequestHandler(
        IMapper mapper,
        IMediator mediator,
        ...
        : CommandHandler(skillcategoryDbContext, mediator),
            IRequestHandler<
                GetBySearchSkillCategoryRequest,
            ...
            >
    {
        ...
        public async Task<Result<Response<IEnumerable<SkillCategoryResponse>>>> Handle(
            GetBySearchSkillCategoryRequest request,
            CancellationToken cancellationToken
        )
        {
            return await GetBySearchSkillCategoryAsync(request)
                .BindAsync(skillcategorys => Task.FromResult(GenerateResponse(skillcategorys)));
        }

        private async Task<Result<Pagination<SkillCategory>>> GetBySearchSkillCategoryAsync(
            GetBySearchSkillCategoryRequest request
        )
        {
            ...
            return !skillcategorys.Rows.Any()
                ? Result<Pagination<SkillCategory>>.Failure(
                    SkillCategoryErrors.NotFound(request.Query.SearchText)
                )
