...
namespace Services.Features.Financials.Customers.UseCases.Queries
{
    public class GetCustomerRequestHandler(
        ...
        : CommandHandler(customerDbContext, mediator),
            IRequestHandler<GetCustomerRequest, Result<Response<IEnumerable<CustomerResponse>>>>
    {
        ...
        private async Task<Result<Pagination<Customer>>> GetCustomerAsync(
            GetCustomerRequest request
        )
        {
            ...
            return !customers.Rows.Any()
                ? Result<Pagination<Customer>>.Failure(CustomerErrors.IsEmpty())
