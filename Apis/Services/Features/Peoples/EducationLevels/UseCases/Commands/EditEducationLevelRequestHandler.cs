...
{
    public class EditEducationLevelRequestHandler(
        ...
        : CommandHandler(educationlevelDbContext, mediator),
            IRequestHandler<EditEducationLevelRequest, Result<Response<EducationLevelResponse>>>
    {
        ...
        public async Task<Result<Response<EducationLevelResponse>>> Handle(
            EditEducationLevelRequest request,
        ...
        )
        {
            return await ValidateRequest(request)
                ...
                .BindAsync(currentEducationLevel =>
                    EditAndSaveEducationLevelAsync(
        ...
        private static Result<EditEducationLevelRequest> ValidateRequest(
            EditEducationLevelRequest request
        )
        {
            return request.Id != request.RequestId
                ? Result<EditEducationLevelRequest>.Failure(
                    EducationLevelErrors.PreConditionFailed(request.RequestId)
                )
                : Result<EditEducationLevelRequest>.Success(request);
        }
        ...
        private async Task<Result<EducationLevel>> EditAndSaveEducationLevelAsync(
            EducationLevel currentEducationLevel,
            EditEducationLevelRequest request,
            CancellationToken cancellationToken
        )
        {
            var editEducationLevel = new EducationLevel(
                request.Id,
                request.Description,
                request.Picture ?? Shared.Infrastructure.Resources.Images.PictureBase64Image,
                currentEducationLevel.CreatedAt.GetValueOrDefault(),
                currentEducationLevel.CreatedBy,
                _dateTimeService.UtcNow,
                _authenticatedUserService.UserId
            );

            editEducationLevel.AddEvent(new EducationLevelEditedEvent(editEducationLevel.Id));

            await ExecuteTransactionAsync(
                () => _educationlevelDbContext.EducationLevels.Update(editEducationLevel),
                editEducationLevel.GetEvents(),
                cancellationToken
            );

            return Result<EducationLevel>.Success(editEducationLevel);
        }
    }
}
