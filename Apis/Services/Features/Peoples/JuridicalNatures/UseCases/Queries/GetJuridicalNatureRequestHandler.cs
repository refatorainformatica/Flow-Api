...
{
    public class GetJuridicalNatureRequestHandler(
        IMapper mapper,
        IMediator mediator,
            ...
            >
    {
        ...
        public async Task<Result<Response<IEnumerable<JuridicalNatureResponse>>>> Handle(
            GetJuridicalNatureRequest request,
            CancellationToken cancellationToken
        )
        {
            return await GetJuridicalNatureAsync(request)
                .BindAsync(juridicalnatures => Task.FromResult(GenerateResponse(juridicalnatures)));
        }

        private async Task<Result<Pagination<JuridicalNature>>> GetJuridicalNatureAsync(
            GetJuridicalNatureRequest request
        )
        {
            ...
            return !juridicalnatures.Rows.Any()
                ? Result<Pagination<JuridicalNature>>.Failure(JuridicalNatureErrors.IsEmpty())
