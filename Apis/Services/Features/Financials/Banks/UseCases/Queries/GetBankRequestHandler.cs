...
{
    public class GetBankRequestHandler(
        IMapper mapper,
        IMediator mediator,
        BankDbContext bankDbContext
    )
        : CommandHandler(bankDbContext, mediator),
            IRequestHandler<GetBankRequest, Result<Response<IEnumerable<BankResponse>>>>
    {
        ...
        public async Task<Result<Response<IEnumerable<BankResponse>>>> Handle(
            GetBankRequest request,
            CancellationToken cancellationToken
        )
        {
            return await GetBankAsync(request)
                .BindAsync(banks => Task.FromResult(GenerateResponse(banks)));
        }

        private async Task<Result<Pagination<Bank>>> GetBankAsync(GetBankRequest request)
        {
            ...
            return !banks.Rows.Any()
                ? Result<Pagination<Bank>>.Failure(BankErrors.IsEmpty())
