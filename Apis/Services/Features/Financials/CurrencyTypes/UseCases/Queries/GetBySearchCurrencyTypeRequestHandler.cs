...
{
    public class GetBySearchCurrencyTypeRequestHandler(
        IMapper mapper,
        IMediator mediator,
        ...
        : CommandHandler(currencytypeDbContext, mediator),
            IRequestHandler<
                GetBySearchCurrencyTypeRequest,
            ...
            >
    {
        ...
        public async Task<Result<Response<IEnumerable<CurrencyTypeResponse>>>> Handle(
            GetBySearchCurrencyTypeRequest request,
            CancellationToken cancellationToken
        )
        {
            return await GetBySearchCurrencyTypeAsync(request)
                .BindAsync(currencytypes => Task.FromResult(GenerateResponse(currencytypes)));
        }

        private async Task<Result<Pagination<CurrencyType>>> GetBySearchCurrencyTypeAsync(
            GetBySearchCurrencyTypeRequest request
        )
        {
            ...
            return !currencytypes.Rows.Any()
                ? Result<Pagination<CurrencyType>>.Failure(
                    CurrencyTypeErrors.NotFound(request.Query.SearchText)
                )
